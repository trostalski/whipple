...
const DashboardCard = (props: DashboardCardProps) => {
  ...
  const { mutate: deleteMutate } = useMutation(
    () => {
      return axios.delete(
        `${process.env.REACT_APP_SERVER_URL}/${apiV1}/workspaces/${workspaceId}/dashboard_cards/${props.cardInfo.id}`
      );
    },
    {
      onSuccess: () => {
  ...
  return (
    ...
    >
      <div className="w-48 pl-2 pt-2">
        <h3 className="text-lg">{props.cardInfo.title}</h3>
        <p className="text-gray-400">{props.cardInfo.info}</p>
      </div>
      <div className="flex flex-col justify-center items-center">
        {!data || data.datasets.length === 0 ? (
          ...
          CardChart(props.cardInfo.chart_type, data!)
        )}
      </div>
